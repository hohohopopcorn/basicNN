...
#define input  4  //amount of inputs
#define output 1  //amount of outputs
int const amount[layers+2]={input, 40, 40, output};  // amount of nodes in layer n
...
double *nodes=(double *)mkl_malloc((layers+2)*MAX*sizeof(double), 128);  //nodes[layers+2][MAX];  //values in each node after networking once (a terms), h(a)=z terms
double *delta=(double *)mkl_malloc((layers+2)*MAX*sizeof(double), 128);  //delta[layers+2][MAX];  //error in node (delta[0][] should all be empty)
...
void backprop(int lay=1)  //back propagation (which layer) first hidden layer is 1
{
	...
	if(lay==layers+1)
	{
		for(int i=1; i<=amount[lay]; i++)delta[lay*MAX+i]=
			errorfunctionprime((nodes[lay*MAX+i]-translation[i-1+input])*stretch[i-1+input],
			(training[input+i-1]-translation[i-1+input])*stretch[i-1+input]);
		delta[lay*MAX]=0;  //bias does not pass error
	}
	else
	{
		cblas_dgemv(CblasRowMajor, CblasNoTrans, MAX, MAX, 1,
			&w[lay*MAX*MAX], MAX, &delta[(lay+1)*MAX], 1, 0, &delta[lay*MAX], 1);
		for(int i=1; i<=amount[lay]; i++)delta[lay*MAX+i]=delta[lay*MAX+i]*hprime(nodes[lay*MAX+i]);
		delta[lay*MAX]=0;  //bias does not pass error
	}
	...
	for(int i=0; i<=amount[lay-1]; i++)
	{
		double j;
		if(i==0)j=1;
		else if(lay-1==0)j=nodes[(lay-1)*MAX+i];
		else j=h(nodes[(lay-1)*MAX+i]);
